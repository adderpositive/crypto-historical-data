...
function processData( currenciesData, generalData ) {
  const requests = [];
  const dataResponse = JSON.parse( currenciesData ).data;
...
export default function init( data ) {
  ...
  request.open('POST', './data-all-currencies', true);
  request.setRequestHeader('Content-Type', 'application/json; charset=UTF-8');
  request.onload = () => {
    if ( request.readyState === request.DONE ) {
      if ( request.status === 200 ) {
  ...
  request.send(
    JSON.stringify({
      amountOfCurrencies: data.amountOfCurrencies
    })
  );
}